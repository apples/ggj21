...
void scene_gameplay::on_connect(const connection_ptr& conn) override {
...
void scene_gameplay::on_disconnect(const connection_ptr& conn, asio::error_code ec) override {
...
void scene_gameplay::on_receive(channel::state_updates, const connection_ptr& conn, std::istream& data) override {
    for (auto& player : current_state.players) {
        if (player.conn == conn) {
...
void scene_gameplay::on_receive(channel::actions, const connection_ptr& conn, std::istream& data) override {
    for (auto& player : current_state.players) {
        if (player.conn == conn) {
            auto msg = receive_message(data);
